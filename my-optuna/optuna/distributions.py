...
class ElementOfDistributionsList(
    NamedTuple('_BaseElementOfDistributionsList',
               [('name', str),
                ('dist', Union[UniformDistribution, LogUniformDistribution])])):

    """An element of distributions list.

    Attributes:
        name:
            A name of this element.
        dist:
            A body of this element.
    """
...
class JointDistribution(
    # TODO(imamura): This impl may contain bag. Be careful.
    NamedTuple(
        '_BaseJointDistribution',
        [('distributions_list', Tuple[ElementOfDistributionsList, ...])]),
    BaseDistribution):
    ...
    Attributes:
        distributions_list:
            A list of distribution members.
    """

    def to_external_repr(self, param_value_in_internal_repr):
        # type: (Dict) -> Sequence[float]

        return [param_value_in_internal_repr[d.name] for d in self.distributions_list]

    def to_internal_repr(self, param_value_in_external_repr):
        # type: (Sequence[float]) -> Dict

        param_dict = {}
        for i, d in enumerate(self.distributions_list):
            param_dict[d.name] = param_value_in_external_repr[i]
...
DISTRIBUTION_CLASSES = (UniformDistribution, LogUniformDistribution,
                        ...
                        CategoricalDistribution, JointDistribution)
