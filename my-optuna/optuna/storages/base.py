...
class BaseStorage(object):
    ...
    @abc.abstractmethod
    def set_sampling_timedelta(self, trial_id, timedelta_sampling_elapsed):
        # type: (int, datetime.timedelta) -> None
    ...
    # Methods for the Time-Varying GPSampler

    def get_trial_elapsed_time(self, study_id):
        # type: (int) -> List[datetime.timedelta]
        ...
        return [
            ...
            for t in all_trials
            if (t.datetime_complete is not None and
                t.state is structs.TrialState.COMPLETE)
        ]

    def get_func_eval_elapsed_time(self, study_id):
        # type: (int) -> List[datetime.timedelta]

        all_trials = self.get_all_trials(study_id)

        for t in all_trials:
            if t.datetime_complete is not None and t.state is structs.TrialState.COMPLETE:
                print(t.datetime_complete, t.datetime_start, t.timedelta_sampling_elapsed)

        return [
            t.datetime_complete - t.datetime_start - t.timedelta_sampling_elapsed
            for t in all_trials
            if (t.datetime_complete is not None and
                t.state is structs.TrialState.COMPLETE)
        ]
