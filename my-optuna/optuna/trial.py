...
class Trial(BaseTrial):
            ...
            if p[0] == 'uniform':
                distributions_list.append(
                    distributions.ElementOfDistributionsList(
                        name=p[1], dist=distributions.UniformDistribution(low=p[2], high=p[3])))
            elif p[0] == 'loguniform':
                distributions_list.append(
                    distributions.ElementOfDistributionsList(
                        name=p[1], dist=distributions.LogUniformDistribution(low=p[2], high=p[3])))
            else:
                raise ValueError('The {} is not implemented for suggest_joint'.format(p[0]))
        distribution = distributions.JointDistribution(distributions_list=distributions_list)

        dict_of_param_value_in_internal_repr = self._call_sampler(name, distribution)

        set_success = True
        for d in distributions_list:
            set_success &= self.storage.set_trial_param(
                self.trial_id, d.name, dict_of_param_value_in_internal_repr[d.name], d.dist)
        set_success &= self.storage.set_trial_param(
            self.trial_id, name, dict_of_param_value_in_internal_repr, distribution)
        if not set_success:
            for d in distributions_list:
                dict_of_param_value_in_internal_repr[d.name] = self.storage.get_trial_param(self.trial_id, d.name)

        list_of_params_value = distribution.to_external_repr(dict_of_param_value_in_internal_repr)
    ...
    def _suggest(self, name, distribution):
        ...
        param_value_in_internal_repr = self._call_sampler(name, distribution)
    ...
    def _call_sampler(self, name, distribution):
        # type: (str, distributions.BaseDistribution) -> Any

        datetime_sampling_start = datetime.now()
        param_value_in_internal_repr = self.study.sampler.sample(
            self.storage, self.study_id, name, distribution)
        datetime_sampling_complete = datetime.now()
        # TODO(imamura): The following method is only implemented for in_maemory storage. Implement it for other ones.
        print("sampling end!. {} {}".format(datetime_sampling_start, datetime_sampling_complete))
        try:
            self.storage.set_sampling_timedelta(self.trial_id, datetime_sampling_complete - datetime_sampling_start)
        except AttributeError:
            print("The sampling time counter is only implemented for in_memory storage.")
