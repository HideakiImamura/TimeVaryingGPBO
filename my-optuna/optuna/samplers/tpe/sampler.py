    ...
    def sample(self, storage, study_id, param_name, param_distribution):
        ...
        if isinstance(param_distribution, distributions.UniformDistribution):
            ...
            return self._sample_categorical(
                param_distribution, below_param_values, above_param_values)
        elif isinstance(param_distribution, distributions.JointDistribution):
            distribution_list = param_distribution.distributions_list
            returned_params = {}
            for d in distribution_list:
                returned_params[d.name] = self.sample(storage, study_id, d.name, d.dist)
            return returned_params
        else:
            distribution_list = [distributions.UniformDistribution.__name__,
                                 ...
                                 distributions.JointDistribution.__name__]
        ...
        if samples.size:
            ...
            if samples.size != score.size:
                raise ValueError("The size of the 'samples' and that of the 'score' "
                                 "should be same. "
                                 "But (samples.size, score.size) = ({}, {})"
                                 .format(samples.size, score.size))
